...
标 † 的是最近一次全量跑之后加入的：每一条都拿那次的 356 条轨迹核对过，也都有
一个"去掉改动就会失败"的测试，但都还没有端到端实测过。
`benchmark/docs/reference-comparison.md` 记录了每一条参考的是哪个 Agent，
以及同一批轨迹否掉了哪些设计。
...
| 单任务的整段运行按完整摘要压缩 † | 一道题就是一"轮"对话，每次切分都落在这一轮中间、走了简短的"轮次前缀"分支：9 次压缩里有 4 次**把约 25 万 token 的工作压成 2,302–4,443 个字符**，走完整提示的是 7,113–13,193 |
...
| 思考被截断的一轮，把思考还给模型 † | 思考不回传，所以"从中断处接着想"指向的东西根本不存在：**60 条轨迹里有 292 轮**思考被截断，其中 43% 紧接着又被截断一次，每次都从头重推同一个方案 |
| 停在思考里的一轮是卡住，不是回答 † | 14 条轨迹里出现 28 次；有 3 次运行就结束在这一轮上，`gpt2-codegolf` 分别在第 1、2、24 分钟，交付文件一直没写出来 |
| 工具调用被截断，同样抬高上限 † | **42 次被截断的工具调用全部**停在初始的 16K，模型自己的 32K 一次没用；其中 15 次要两轮以上才重新发成功 |
...
| 截断的命令输出同时保留开头和结尾 † | 只留结尾会丢掉第一个编译错误；71 次截断里有 20 次，模型接着就回头去找被丢掉的部分 |

### 提示词

| 改动 | 促成它的测量 |
|---|---|
| 告诉模型它的思考不会被保留 † | 工具调用旁边的可见正文**中位长度是 0 个字符**；而每轮思考中位 1,016–1,349 字符，在之后的每次请求里都被丢掉 |
...
**只有判分器真正跑了，0 分才算 Agent 的。** 最近四个 arm 里有 14 个 0 分，
其实是判分器自己的测试工具没装上——有两道题在这个环境里永远拿不到分；有一个
arm 因为一次 GitHub 网络故障，看起来像 p = 0.057 的退步，把这些排除之后是
p = 0.73。每个 trial 的时间预算跟随 harbor 给这道题的实际时限（80 到 1,600
分钟不等），而不是所有题一个数。

`benchmark/docs/` 保留了所有被测量并否决掉的候选解释，而那是其中的大多数
——目前三十四个，其中好几个是已经写完了的设计。
